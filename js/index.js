...
var app = {
    ...
    init: function () {
		...
		function initPages () {
			...
			$( document ).on( "swipeleft swiperight", "#list li", function( event ) {
				...
				var coupon_id = $( this ).attr("delete-id");
				//alert(coupon_id+" : "+transition);
				confirmAndDelete_coupon( coupon_id ,listitem, transition );
			});
						
			//}
			function confirmAndDelete_coupon( coupon_id ,listitem, transition ) {
			...
			$('#form1').validate({
				...
				submitHandler: function (form) {					
					...
					$.ajax({
						...
						data: { uuid: device.uuid, name: $("#f_name").val(), phone: $("#mob_no").val(), email: $("#email").val(), ref_merchant: $("#merchant_name").val(), city: $("#city_name").val() },
						type: "POST",
						success: function(data) {
    ...
    cityFill: function() {
		$.ajax({
			...
			url: web_url+"users/list_city.php",			
			type: "POST",
			success: function(data) {
				//alert(data);
				cb = '';
...
                $.each( response, function ( i, val ) {
					cb+='<option value="'+val.id+'">'+val.name+'</option>';
                });
				$("#city_name").append(cb);
				$("#city_name").selectmenu( "refresh", true );				
			}
		});
	},
